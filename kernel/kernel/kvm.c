...
SegDesc
    gdt[NR_SEGMENTS];  // the new GDT, NR_SEGMENTS=7, defined in x86/memory.h
TSS tss;
...
void init_vm() {
    for (uint32_t i = 0; i < PHY_MEM / PT_SIZE; ++i) {
        ...
        for (uint32_t j = 0; j < NR_PTE; ++j) {
            kptable[i * NR_PTE + j].val =
                MAKE_PTE((i * NR_PTE + j) * PAGE_SIZE, PTE_W);
        }
    }
...
/** The function uvm_protect creates a new page 
directory for a user process by allocating memory 
for it and initializing it with the kernel's page directory entries.
 */
PDE* uvm_protect() {
    PDE* pgdir = kalloc();
        ...
        if (src[i].present) {
            ...
            for (int j = 0; j < NR_PTE; ++j) {
                if (src_pte[j].present) {
                    void *src_m = (void*)(src_pte[j].page_frame << 12),
                         *dst_m = kalloc();
