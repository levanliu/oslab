...
void load_proc(TrapFrame* ctx, PDE* pgtbl, uint32_t entry,
               uint32_t ustack_top) {
...
/**
CR3 Register
The CR3 register in x86 architecture holds the physical address
of the page directory base.
It is used by the CPU's memory management unit (MMU)
...
void sys_clone(TrapFrame* tf) {
    ...
    // Share the address space with the parent process
    p->ctx->cr3 = curr->ctx->cr3;

    // Copy the trap frame to the new thread
    *(p->ctx) = *tf;

    // Set up the new thread's stack
    p->ctx->esp = (uint32_t)kalloc();  // Allocate a new stack for the thread
    if (!p->ctx->esp) {
        tf->eax = -1;
        return;
    }
    memcpy((void*)p->ctx->esp, (void*)tf->esp, PAGE_SIZE);  // Copy the stack

    // Set the return value for the new thread
    p->ctx->eax = 0;
    ...
    // Set the new thread's status to RUNNABLE
    push_off();
    p->status = RUNNABLE;
    pop_off();
}
...
void sys_wait(TrapFrame* tf) {
    ...
    push_off();
    for (;;) {
        for (proc_t* p = proc_pool; !found && p < &proc_pool[MAX_PROC_NUM];
             ++p) {
            if (p->status != UNUSED && p->parent == curr) {
                ...
                if (p->status == ZOMBIE) {
                    ...
                    tf->eax = p->pid;
                    if (tf->GPR1)
                        *(int*)(tf->GPR1) = p->exit_code;
                    p->parent = NULL;
                    uvm_dealloc((PDE*)p->ctx->cr3, PHY_MEM,
                                USER_STACK_TOP - PHY_MEM);
...
void sys_exec(TrapFrame* tf) {
    PDE *pgdir, *currpg;
    uint32_t entry, ustack = USER_STACK_TOP;
    if (load_elf((char*)tf->GPR1, (char**)tf->GPR2, &pgdir, &entry, &ustack,
                 STACK_SIZE) < 0) {
        tf->eax = -1;
        return;
    }
...
void sys_kill(TrapFrame* tf) {
    ...
    for (proc_t* p = proc_pool; p < &proc_pool[MAX_PROC_NUM]; ++p) {
        push_off();
        if (p->status != UNUSED && p->status != UNINIT && p->status != ZOMBIE &&
            p->pid == pid) {
            p->killed = 1;
            found = 1;
...
void sys_brk(TrapFrame* tf) {
    ...
    if (curr->brk == 0) {
        ...
        if (new_brk > curr->brk) {
            uvm_alloc((PDE*)read_cr3(), curr->brk, PAGE_UP(new_brk) - curr->brk,
                      PTE_U | PTE_W);
            curr->brk = PAGE_UP(new_brk);
        } else if (PAGE_UP(new_brk) < curr->brk) {
            uvm_dealloc((PDE*)read_cr3(), PAGE_UP(new_brk),
                        curr->brk - PAGE_UP(new_brk));
            curr->brk = PAGE_UP(new_brk);
        }
        tf->eax = 0;
